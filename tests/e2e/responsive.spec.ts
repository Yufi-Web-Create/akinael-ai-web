...
for (const viewport of viewports) {
  test(`home is usable at ${viewport.width}x${viewport.height}`, async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole("heading", { name: "相談から制作・運用まで、まとめておまかせ" })).toBeVisible();
    await expect(page.getByRole("heading", { name: "小さな商いでも、続けやすい価格に" })).toBeVisible();
...
for (const path of industryPaths) {
  test(`${path} is usable at mobile and desktop widths`, async ({ page }, testInfo) => {
    ...
    await testInfo.attach(`${path.split("/").filter(Boolean).pop()}-390x844`, { body: await page.screenshot({ fullPage: true }), contentType: "image/png" });
    await page.setViewportSize({ width: 1440, height: 900 });
    await assertUsablePage(page, path);
    await testInfo.attach(`${path.split("/").filter(Boolean).pop()}-1440x900`, { body: await page.screenshot({ fullPage: true }), contentType: "image/png" });
  });
}

test("homepage reflects the production LP design system", async ({ page }) => {
  await page.goto("/", { waitUntil: "networkidle" });
  await expect(page.locator(".landing-v2")).toHaveCount(1);
  await expect(page.locator(".cmyk-head .plate-c")).toHaveCount(2);
  await expect(page.locator(".cmyk-head .plate-m")).toHaveCount(2);
  await expect(page.locator(".cmyk-head .plate-y")).toHaveCount(2);
  await expect(page.locator("[data-slider] img")).toHaveCount(6);
...
test("internal navigation links resolve without 404s", async ({ page, request }) => {
  await page.goto("/", { waitUntil: "networkidle" });
  const hrefs = await page.$$eval("a[href]", (anchors) => anchors.map((a) => a.getAttribute("href") || "").filter((href) => href.startsWith("/") && !href.startsWith("//")));
...
test("register widget rejects a short password before calling the API", async ({ page }) => {
  ...
  page.on("request", (req) => { if (req.url().includes("/api/v2/auth/register")) requests.push(req.url()); });
  await page.goto("/", { waitUntil: "networkidle" });
...
test("register widget requires acknowledgement before calling the API", async ({ page }) => {
  ...
  await widget.locator("button[type=submit]").click();
  expect(await widget.locator('input[name="consent"]').evaluate((input: HTMLInputElement) => input.validity.valueMissing)).toBe(true);
  expect(requests).toEqual([]);
});
...
test("register widget follows the Core API success contract", async ({ page }) => {
  await page.route("https://akinael-ai.com/api/v2/auth/register", async (route) => {
    ...
    await route.fulfill({ status: 201, contentType: "application/json", headers: { "access-control-allow-origin": "*" }, body: JSON.stringify({ token: "e2e-browser-token" }) });
  });
  await page.route("https://akinael-ai.com/portal/", async (route) => { await route.fulfill({ status: 200, contentType: "text/html", body: "<title>Portal destination</title>" }); });
  await page.goto("/", { waitUntil: "networkidle" });
  const widget = page.locator("#register-home");
